...
### 2. Слои кэширования в реальных системах
   - Браузерный кэш / HTTP‑кэш.
     - Держит «то, что близко к пользователю»: статику (JS, CSS, картинки) и иногда ответы API.
     - Управляется в основном через HTTP‑заголовки и версионирование ресурсов; подробности можно разобрать позже, если это важно для примера.
   ...
   - В сценарии важно показать одну общую схему слоёв (её удобно рисовать, например, в Excalidraw); подробности про HTTP‑кэш и Service Worker можно держать как запасной материал.
...
5. **Механизмы обеспечения согласованности (2–3 базовые стратегии)**
   ...
   - Показываем, как выбор стратегии записи (cache-aside, read-/write-through, write-behind) влияет на согласованность между уровнями кэша и БД.
     - Коротко освежаем сами паттерны:
       - cache-aside (lazy loading) — самый частый, сервис сам идёт в БД при miss и кладёт результат в кэш;
       - read-through / write-through — кэш сам ходит в БД, каждое чтение/запись проходит через него;
       - write-behind — запись сначала в кэш, затем асинхронный сброс в БД.
     - На схеме и в псевдокоде в сценарии разбираем подробно именно cache-aside, остальные паттерны оставляем как варианты.
