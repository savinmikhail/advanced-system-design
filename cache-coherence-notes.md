...
### 1. Кэш как часть архитектуры, а не просто «ускоритель»
   - Зачем вообще всё это городить: разгрузка основной БД и внешних API, ускорение горячих чтений (карточка пользователя, список ближайших объектов, состояние заказа), переиспользование дорогих вычислений (агрегации, рекомендации, отчёты), отдельные кейсы вроде rate limiting и feature‑флагов.
   ...
   - Базовый паттерн взаимодействия — cache-aside (lazy loading); дальше почти все примеры будут строиться вокруг него.
