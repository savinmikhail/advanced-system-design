...
В прошлых частях мы постоянно улучшали систему инженерными приёмами: кэшировали, добавляли реплики, 
уходили от транзакций к сагам, принимали eventual consistency. 
...
- повышаем надёжность — растут задержки; 
...
На уровне данных обычно компромисс между скоростью чтений, скоростью записей и строгостью консистентности:

- реплики ускоряют чтения, но дают replication lag;
- индексы/materialized views ускоряют SELECT, но замедляют write и усложняют модель.
- ![img_6.png](img_6.png) синхронные подтверждения/кворумы усиливают консистентность, но повышают latency и tail latency

То есть если:

- ускоряем чтение — почти всегда платим записью и/или строгой консистентностью;
- усиливаем консистентность — платим latency и теряем throughput.
...
Но в реальности появляется эффект noisy neighbor: различные background джобы съедают ресурсы из-за чего latency у пользовательского API страдает. 
при этом это могут быть даже не наши джобы если мы на виртуалке
