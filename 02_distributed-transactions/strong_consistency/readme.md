# Сценарий: Strong Consistency в распределённых системах (12–15 минут)

## Вступление: от eventual к strong

В прошлой части мы смотрели на ту же историю с заказом, платежами и инвентарём,
но через призму распределённых транзакций и саг: мирились с тем, что между сервисами
какое‑то время будет рассинхрон и придётся жить на eventual consistency.

В этом блоке двигаемся к противоположному полюсу —
strong consistency: когда вы готовы платить latency, деньгами и сложностью
за то, чтобы после успешной записи все чтения этого же ключа видели одно и то же значение.

## 1. Что такое Strong Consistency по-человечески
...
Linearizability — одна из самых строгих и дорогих по стоимости гарантий, которые можно себе позволить в распределённой системе.
...
* **Операции без вменяемой компенсации**:
    ...
    * транзакции, которые ты не можешь “откатить событиями”.
...
Полезно знать, что такое вообще возможно, но это не “паттерн на выходные”.
...
* профили пользователя (описание, аватар, соцсетки);
* ленты новостей;
* лайки, реакции, счётчики “просмотров”;
* корзина покупок;
* каталоги товаров;
