...
### Блок 3.1. Распределённые транзакции: от 2PC до Saga

#### Вступление (1–2 мин)

Мы уже порезали данные по шардам, научились переносить их между нодами и переживать рост нагрузки.
Но бизнес-процессы от этого никуда не делись: деньги нужно списывать, товар резервировать, заказы создавать.
И всё это одновременно трогает несколько сервисов и несколько баз.

На этом месте простое горизонтальное масштабирование упирается в новый класс вопросов:

* как сделать так, чтобы деньги списались **ровно один раз**, даже если участвуют несколько сервисов и БД;
* как не потерять заказ, если один из шагов цепочки лёг посередине;
* как жить с тем, что данные, разбросанные по шардам, всё равно должны выглядеть для пользователя “цельными”.

Вот здесь и начинаются распределённые транзакции — не как красивая теория,
а как попытка хоть как‑то навести порядок в этих сценариях.
...
Здесь привычный ACID в рамках одной БД перестает работать,
а фраза “просто откатим всё назад” — это уже не архитектура, а признак того, что мышление осталось на уровне монолита.
...
#### 3.1.1. Почему транзакции ломаются в микросервисах (3–4 мин)
...
#### 3.1.2. Классика: Two-Phase Commit (2PC) (4–5 мин)
...
Сделать рабочий 2PC в проде **можно**, если вы — Google/Meta и пишете что-то уровня Spanner.
...
> но для большинства продуктовых команд — решение, которое слишком дорого и редко оправдано.

И это как раз тот вопрос, который любят задавать на собесах:

> “Почему нельзя просто сделать 2PC между микросервисами?”

Хороший ответ — это не “ну сложно”, а перечисление конкретных проблем:

* глобальные блокировки на фазе `PREPARE`, которые убивают latency и throughput;
* координатор как single point of failure;
* завязка tail latency на самый медленный участник и на сеть;
* хрупкость под реальными таймаутами, GC-паузами и рестартами.

Практически 2PC чаще всего живёт:

* либо внутри одной СУБД / кластера (XA, распределённые транзакции SQL-серверов);
* либо в “тяжёлых” системах уровня Spanner/Cockroach, где протокол спрятан внутри базы.

На уровне приложения это не “магический флаг”, а конкретные протоколы и API:

* XA/JTA в мире Java (application-сервер как координатор, БД и брокер как участники);
* протоколы logical replication / распределённых транзакций в конкретной БД.

Важно уметь честно сказать:
“да, есть готовые механизмы 2PC внутри баз и transaction manager’ов, но тянуть их поверх зоопарка микросервисов — почти всегда плохая идея”.

---

#### 3.1.3. Three-Phase Commit (3PC) — знать идею (1 минута)

3PC — это попытка “починить” 2PC добавлением ещё одной фазы `preCommit`,
чтобы участники могли принять решение (коммит/rollback) даже если координатор умер после `preCommit`,
а не зависать бесконечно, как в 2PC после `PREPARE`.

Отличие от 2PC в том, что у нас появляется промежуточное состояние “почти закоммитили”, и протокол пытается быть неблокирующим.
Но цена — сильные предположения про сеть и время:
нужны предсказуемые таймауты, ограниченные задержки и возможность отличить “узел умер” от “пакет потерялся”.

В реальных облаках с асинхронной сетью всё это разваливается, поэтому 3PC почти не встречается в проде:
для практики достаточно знать, что это “2PC с дополнительной фазой”, пригодный в основном для учебников.

---

#### 3.1.4. Saga — как это делают нормальные системы (10–12 мин)
...
## Канонический сценарий e-commerce от начала до конца

Давайте один раз “проживём” сагу целиком на примере маркетплейса.

Сценарий: пользователь оформляет заказ.

1. **Создаём “черновик” заказа** в `Orders`:

    * статус `PENDING`;
    * сохраняем `orderId` и `sagaId`.

2. **Шаг 1: списать деньги** (`Billing`):

    * проверяем лимиты и баланс;
    * делаем hold/списание с `operationId = sagaId + "/billing"`;
    * пишем в свою БД и в outbox событие `BillingSucceeded`/`BillingFailed`.

3. **Шаг 2: зарезервировать товар** (`Inventory`):

    * на событие `BillingSucceeded` инвентарь пытается зарезервировать позиции заказа;
    * если ок — пишет `InventoryReserved`;
    * если не ок (нет остатков) — `InventoryFailed`.

4. **Шаг 3: финализировать заказ** (`Orders`):

    * на `InventoryReserved` меняем статус заказа на `CONFIRMED`;
    * на `InventoryFailed` — на `CANCELLED` и запускаем компенсацию по биллингу.

5. **Шаг 4: уведомления** (`Notifications`):

    * слушает события `OrderConfirmed`/`OrderCancelled`;
    * шлёт пуши, письма, события в аналитику.

Компенсации в этом сценарии:

* если упал `Inventory` после успешного биллинга — `Billing` делает `refund`/release hold по тому же `operationId`;
* если упал `Orders` после резервирования — при повторном получении событий `InventoryReserved`/`InventoryFailed` мы проверяем `order.status` и аккуратно доводим его до нужного состояния.

Ключевой момент: нигде нет “глобальной транзакции на всё” —
есть отдельные локальные транзакции в своих БД + события + компенсирующие шаги.
Дальше мы подкладываем под это Outbox+CDC и идемпотентность, чтобы вся эта красота выдерживала ретраи, повторы и падения.
...
#### 3.1.5. Оркестрация против хореографии (4–5 мин)
...
#### 3.1.6. Temporal, Camunda, Conductor и прочие движки (5–6 мин)
...
* вы описываете workflow как обычный код (функция, которая вызывает “активити”);
* Temporal сам:
...
Если вернуться к нашему e-commerce сценарию:

* `Billing` в одной транзакции пишет `payment` и запись в `outbox` с событием `BillingSucceeded/BillingFailed`;
* `Inventory` слушает эти события из Kafka и тоже через outbox публикует свои `InventoryReserved/InventoryFailed`;
* `Orders` и `Notifications` реагируют на поток событий и меняют свои состояния.

С точки зрения приложения это выглядит не как “магический брокер”, а как очень приземлённая работа:

* добавить таблицу `outbox` (иногда несколько, по типам событий);
* оборачивать каждую бизнес-транзакцию “записью в outbox”;
* поднять коннектор (Debezium/самописный воркер), который читает outbox и пишет в брокер с гарантиями.

---

#### 3.1.8. Idempotency (3–4 мин)
...
В контексте нашей саги это означает:

* каждый шаг (`Billing`, `Inventory`, `Orders`) должен уметь безопасно обрабатывать одно и то же событие/запрос несколько раз;
* каждое событие в outbox помечено ключом идемпотентности (тот же `operationId` или `(sagaId, step)`), по которому сервис понимает “это уже применяли”;
* компенсирующие операции (`refund`, `releaseInventory`) тоже идемпотентны: повторный вызов не делает хуже.

Три кита рабочей распределённой транзакции в микросервисах:

1. Saga как сценарий из шагов и компенсаций.
2. Outbox+CDC как способ доставить события надёжно и в правильном порядке.
3. Idempotency как страховка от повтора шагов и сообщений.

---

#### 3.1.9. Типичные фейлы распределённых транзакций (3–5 мин)
...
#### 3.1.10. Когда Saga — плохая идея (2 мин)
...
#### 3.1.11. Альтернативы и дополнения к Saga (2–3 мин)
...
Всё это работает только потому, что мы согласились на модель,
где разные части системы какое-то время видят разные версии правды.
Формально это называется eventual consistency
