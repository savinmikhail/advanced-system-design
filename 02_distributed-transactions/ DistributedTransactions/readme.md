...
## Блок 3. Three-Phase Commit (3PC) — попытка починить 2PC (1–2 мин)

После 2PC логичный шаг был такой:

> “Окей, 2PC блокирует всё при падении координатора.
> Давайте добавим ещё одну фазу и сделаем протокол неблокирующим.”

Идея 3PC очень упрощённо:

1. **canCommit?**
   Координатор спрашивает участников: “Вы вообще в принципе готовы к транзакции?”

2. **preCommit**
   Если все сказали “да” — координатор рассылает `PRE_COMMIT`:

   * участники фиксируют, что “мы на грани коммита”;
   * но пока ещё не сделали `COMMIT`.

3. **doCommit**
   После этого координатор рассылает `DO_COMMIT`,
   и участники действительно коммитят.

Фишка:

* в 2PC участники после `PREPARE` висят в непонятном состоянии и зависят от координатора;
* в 3PC в теории, если координатор умер после `PRE_COMMIT`, участники,
  **используя таймауты и свои состояния**, могут сами принять решение — закоммитить или откатить,
  не залипая навсегда в “подвешенном” состоянии.

Звучит красиво: “неблокирующий commit-протокол”.

Проблема в том, **на чём это держится**:

* на предположении, что:

  * сеть даёт предсказуемые таймауты,
  * мы можем отличить “узел умер” от “пакет потерялся”,
  * задержки ограничены сверху.

В реальном облаке:

* сеть асинхронная;
* пакеты теряются и приходят с дикими задержками;
* GC/стоп-мир/рестарты выглядят точно так же, как “узел умер”.

И 3PC попадает ровно в ту же классическую теорию (FLP и всё такое):

> в асинхронной системе с падениями узлов нельзя построить надёжный согласованный протокол,
> который гарантированно и завершится, и не развалит консистентность.

По факту:

* 3PC сложнее 2PC;
* всё равно опирается на нереалистичные предположения про сеть и таймауты;
* почти не встречается в реальных продах.

Поэтому для практики тебе достаточно знать:

> 3PC — это “2PC с промежуточным pre-commit”,
> который на теории смотрится лучше,
> но в реальной асинхронной распределённой системе остаётся игрушкой для учебников.
