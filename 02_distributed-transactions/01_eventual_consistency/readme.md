...
Вместе с данными в той же транзакции мы записываем и сообщение, которое надо будет отправить, — в специальную таблицу `outbox`.

Затем отдельный процесс читает `outbox` и публикует события.

Так мы избегаем ситуации “событие отправили, а данные не сохранились” (или наоборот).
