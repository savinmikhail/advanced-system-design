...
Саги и outbox неизбежно приводят нас к **eventual consistency**: деньги, товар, заказы и уведомления перестают жить в одной транзакции и обновляются с задержкой относительно друг друга.
...
Все межсервисные события должны рождаться:

* в одной локальной транзакции вместе с изменением данных (`entity + outbox`);
* через CDC/коннектор, который гарантированно доставляет события в брокер.

Без этого любые разговоры про консистентность между сервисами бессмысленны: события будут теряться и дублироваться без контроля.

### 6.5. Идемпотентность везде, где есть сеть

Любая операция, которую могут повторить (ретраи, дубликаты сообщений), должна уметь:

* по ключу (`operation_id`, `(saga_id, step)` и т.п.) понять, что она уже выполнялась;
* либо ничего не делать, либо возвращать тот же результат.

Это единственный реалистичный способ жить с at-least-once и reorder.

### 6.6. Ретраи с backoff и ограничениями

Ретраи нужны, но:

* без backoff вы превращаете временную деградацию в DDOS по своей же системе;
...
Вместе с данными в той же транзакции мы записываем и сообщение которое надо будет обработать в специальную таблицу

Затем отдельный процесс читает `outbox` и публикует события;

Таким образом мы избегаем ситуации когда событие опубликова - данные не сохранили или наоборот
...
* есть write-модель (ledger заказов/платежей) с максимально строгими инвариантами;
