...
В прошлой части мы разбирали историю с заказом, платежами и инвентарём:
хотели списать деньги, зарезервировать товар, создать заказ и разослать уведомления так,
чтобы это выглядело как одна логическая операция.

Там мы честно признали, что глобального ACID поверх микросервисов не будет,
и перешли к сага‑подходу: последовательности шагов с компенсирующими действиями
и сознательным допущением eventual consistency между сервисами.

В этой части смотрим на ту же картинку под другим углом:
что именно означает “данные расходятся во времени”, какие эффекты даёт такая модель консистентности
и где она уже живёт в вашей системе, даже если вы про это формально не договаривались.
...
Если ты пытаешься играть в “глобальный ACID поверх зоопарка сервисов”,
ты воюешь не с фреймворком, а с физикой.
...
## 4. Где eventual consistency уже живёт, даже если ты “не подписывался”
...
* во временные окна деградации ты забиваешь и так мучающуюся систему.
...
* вводишь асинхронную репликацию;
* ставишь очереди;
* используешь кеши с TTL;
* шардишь данные по разным базам;

— ты выбираешь **меньше блокировок и выше доступность**
