...
* в сервисе `orders` запись уже есть;
* в `inventory` зарезервировать товар ещё не успели;
* `billing` задержался с подтверждением списания;
* сервис уведомлений уже выплюнул письмо “Заказ создан”;
* в аналитике этот заказ появится через минуту;
* в поиске его не видно ещё пять.

Пользователь открывает “Мои заказы” и спрашивает:

> “Где мой заказ?!”

Ты смотришь в лог `orders` и видишь:

> “Он есть. Просто не везде. Он… eventual.”

Вот эта щель между “данные уже записаны в одном месте” и
“данные согласованы во всех нужных местах” — и есть пространство eventual consistency.
Глава ровно про то, **как с этим жить, не сойти с ума и не строить иллюзий про магический ACID по микросервисам.**
