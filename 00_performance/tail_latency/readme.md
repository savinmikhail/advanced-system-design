## Блок 1.1. Tail Latency: откуда берутся хвосты и как с ними жить

В предыдущей части мы договорились про базовые понятия latency и throughput
и увидели, что “средние” значения в отчётах почти ничего не говорят пользователю:
его волнует ровно его запрос, который иногда почему‑то едет 3 секунды.

Сейчас копнём именно хвост — p95/p99, те самые редкие, но очень болезненные запросы,
которые формально “укладываются в SLA”, но на практике ломают всё впечатление от системы.
...
есть эффект noisy neighbor — когда другой сервис на той же ноде начинает жрать ресурсы и убивает вам p99
...
Самые неприятные эффекты начинаются, когда система **пытается “умно” на него отреагировать**:
