...
как увеличить throughput? кажется все просто - масштабировать систему горизонтально, то есть добавлять больше серверов
...
#### 3. Gustafson: масштабируем не скорость, а объём работы

Но бизнесу не всегда нужно “сделать то же самое быстрее”.

Часто вопрос звучит иначе:

> У нас есть фиксированный бюджет времени (например, 200 ms p95 или 1 час ночного джоба).
> Сколько работы мы можем успеть сделать за это время, если добавим ресурсов?

Вот это уже взгляд закона Густафсона.

- Амдаль — про “ускорить один и тот же запрос”.
- Густафсон — про “за то же время обработать больше запросов или более тяжёлую задачу”.

Инженерно это выглядит так:

- нам важнее при том же p95 пережёвывать уже не 10k запросов, а 100k;
- или не просто “отчёт за вчера”, а отчёт за месяц, по всем городам и устройствам, всё равно за тот же SLA.

Как и в случае с амдалем мы не получаем линейного ускорения от добавления серверов, но формула уже другая

Формула Густафсона для ускорения при `N` узлах:
...
- последовательная часть = 10% (`α = 0.1`);
- ставим `N = 8` узлов:
...
- идеал был бы `8×`,
- реально мы можем сделать работы примерно в 7.3 раза больше за то же время (10% всё ещё остаются узким местом).

На практике это читается так:

- масштабируя кластер, мы обычно:

  - либо держим тот же p95, но перевариваем больше RPS;
  - либо оставляем тот же SLA ночного джоба, но считаем более жирную аналитику;
- при этом всё равно упираемся в α — долю общей, последовательной части, которую не шарднули и не разобрали.

Кейс: nightly-отчёт по логам маркетплейса

- Есть job, который за 1 час считает агрегаты по логам заказов за вчера: выручка по магазинам, категориям и т.п.
- Мы добавили ещё N воркеров и масштабировали хранилище логов.
- SLA не поменялся: как был час, так и остался час.
- Вместо “давайте посчитаем тот же отчёт за 10 минут” бизнес обычно говорит:
  “давайте тогда за тот же час:
...
В терминах Густафсона:

- мы не уменьшаем latency отчёта,
- мы увеличиваем объём работы, который умеем делать в тот же временной бюджет.

##### Как это продать на собеседовании

Когда на системдизайне спрашивают:

> “Мы добавили ещё 10 серверов, что это нам даёт?”

Сильный ответ не “ну, будет в 10 раз быстрее”, а примерно такой:

- “По Амдалю ускорение ограничено непараллелящейся частью — оно не будет линейным.”
- “Но по Густафсону мы можем за тот же SLA:

  - либо переваривать больше RPS,
  - либо считать более тяжёлые отчёты / более детальную аналитику.”

То есть ты показываешь, что:

- понимаешь разницу ‘быстрее один запрос’ vs ‘больше работы за то же время’;
- и умеешь говорить о масштабировании не в терминах магического “10х”, а в терминах конкретного бюджета времени и реальной полезной работы.
...
Но в реальных системах при росте числа узлов мы начинаем платить за contention и coherence

- contention — это конкуренция за общие ресурсы, например
...
- coherence — это согласованное состояния между репликами или слоями системы, например
...
Какие выводы можно сделать из этого закона?
...
- при большом N κ · N · (N - 1) становится огромным — затраты на согласование и координацию растут быстрее, чем полезный throughput,
...
Что мы можем с этим поделать? во-первых, по закону литтла все приемы уменьшения latency, которые мы разобрали ранее, помогают нам увеличить throughput

Но есть еще пара дополнительных моментов  1- Разделение нагрузок и данных и 2 - Изоляция hot ресурсов
