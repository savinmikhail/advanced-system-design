...
> Ускорение системы ограничено той частью, которую можно распараллелить.
...
- хоть 100 серверов поставь, если 30% запроса упираются в один последовательный кусок — ты никогда не ускоришь систему больше чем в 3.3 раза.
...
Кейс: централизованный биллинг маркетплейса.

- Все заказы, оплаты и возвраты проходят через один `BillingService` и одну базу платежей.
- Мы добавили ещё фронтовые ноды, разнесли каталог и поиск по шардам — эти части масштабируются почти линейно.
- Но каждый чек‑аут всё равно ждёт ответ от этого единственного биллинга; условные 30% времени запроса уходят туда.
- В итоге p95 создания заказа упирается именно в этот последовательный кусок: по закону Амдаля ускорение ограничено ~3×,
  как бы мы ни наращивали количество фронтов и “лёгких” сервисов.
- Чтобы пробить потолок, мало “добавить серверов” — нужно дробить централизованный биллинг (отдельные инстансы по регионам, мерчантам, видам операций и т.п.).
