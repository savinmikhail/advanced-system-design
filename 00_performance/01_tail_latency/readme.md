## Блок 1.1. Tail Latency

#### 1.1.0. Откуда берутся хвосты

Но откуда берутся хвосты? Они же по определению случаются иногда
...
#### 3. Как ретраи превращают один хвост в retry storm
...
1. Уменьшение хвоста - Сделать так, чтобы медленных запросов становилось меньше.

2. Маскировка/обход хвоста - Сделать так, чтобы редкие медленные запросы не ломали всю систему*- и пользователь видел более стабильное поведение.
