...
Мы разобрали пример, где база не была узким местом
...
Из-за этого Seq scan занимает минуты, долгие бэкапы, vacuum никогда не успевает отработать
...
Репликация — это копии одних и тех же данных: есть primary нода и одна или несколько реплик.
...
- синхронная репликация повышает latency записи - надо ждать пока все реплики скажут, что записали данные
...
- от старых данных при необходимости можем избавиться просто DROP'ом таблицы, а не DELETE'ом с последующим вакуумом, что дешевле
...
К примеру в Postgres `UNIQUE` constraint или `PRIMARY KEY` на родительской таблице должны включать ключ партиционирования, 
потому что глобального индекса по всем партициям нет.
...
Например, гитхаб имел в среднем аж 950к запросов в секунду на монолитной базе, прежде чем решился переходить к шардированию
