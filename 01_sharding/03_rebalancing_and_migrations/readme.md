...
1. Первый шаг - **Backfill (переливание)**  
   ...
   * логируем прогресс (offsет/LSN/последний id).
   ...
   Важно: Postgres не хранит WAL бесконечно. Чтобы гарантировать, что мы сможем дочитать все изменения после snapshot’а, используется replication slot: пока мигратор не применил события до нужного LSN, соответствующие WAL-записи не будут удалены. То есть мы не полагаемся на ‘вечный журнал’, мы организуем управляемый поток изменений
