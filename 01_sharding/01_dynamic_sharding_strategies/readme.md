...
* даже если shard key в целом выбран нормально,
* один такой ключ может фактически **превратить целый шард в bottleneck**:
...
* **hot-key isolation** — вынос особо горячих ключей на отдельный шард/кластер;
* **microsharding** — разбивка шарда на много мелких логических кусочков, чтобы нагрузку можно было перераспределять тонко;
* **дополнительный кеш / fan-out через очередь** — не бить по базе/основному сервису каждый раз напрямую;
* **rate limiting / backpressure** для особо шумных ключей — чтобы один клиент не мог сжечь весь кластер.

Важно: hot keys не лечатся “ещё одним % N”.  
Их нужно **выявлять метриками** и отдельно с ними работать — как с особыми, а не как с обычными ключами.
