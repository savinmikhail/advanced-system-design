...
| Плюсы | Минусы |
|-------|--------|
| ✅ Полный контроль над логикой | ❌ Легко забыть инвалидировать ключ |
| ✅ Простая реализация для простых случаев | ❌ Race conditions при update |
| ✅ Гибкость (можно кэшировать что угодно) | ❌ Код размазан по всему сервису |
| ✅ Популярный паттерн, много примеров | ❌ При 10 слоях — кошмар поддержки |

### Trade-offs

| Критерий | Оценка |
|----------|--------|
| **Время согласованности** | 0-5 секунд (зависит от TTL + события) |
| **Цена реализации** | Низкая (просто Redis + код) |
| **Сложность поддержки** | Высокая при росте (много ключей) |
| **Бизнес-требования** | Подходит для eventual consistency |

### Когда использовать

✅ **Хорошо для:**
- Простых случаев (1-3 ключа)
- Eventual consistency допустима (каталог, ленты)
- Нужна гибкость (нестандартная логика)

❌ **Плохо для:**
- Много связанных ключей (адрес в 10+ местах)
- Strong consistency нужна (балансы, лимиты)
- Критичные race conditions (финансовые операции)
