...
Cache-aside работает, пока у вас две страницы, потом становится все сложнее этим управлять, так как логика размазана по коду
...
такой подход называется  Read-Through
...
Таким образом согласованнасть у нас почти идеальная, но стоимость такого решения высокая потому что высокая latency
...
Есть даже известная фраза Фила Карлтона сказанная еще в прошлом веке
...
Самый простой, но самый неэффективный подход - TTL-only
При этом подходе мы вообще не инвалидируем кэш явно, а просто ждем пока его срок жизни истечет. 
Зато нам не нужно писать лишний код или придумывать какую-то архитектуру
...
как мы только что сказали, подписчики умеют инвалидирвоать связанные ключи
...
а потом получать все ключи с этим тегом и удалять их
...
Мы разобрали базовую механику coherence: где кэш лежит, как он наполняется, и как мы его инвалидируем, чтобы разные слои не расходились слишком сильно.

Но в реальной жизни всплывают **разные аварийные сценарии** — hot keys, thundering herd, race conditions, stale/forever cache, split-brain при блокировках, несовместимость релизов из-за форматов кэша, cache poisoning и прочие проблемы.

Чтобы не раздувать основной материал, эти случаи и практические приёмы защиты я вынес в отдельный разбор на Boosty.

А здесь двигаемся дальше к компромиссам оптимизации ресурсов
