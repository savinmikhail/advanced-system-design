...
и первая из них - race condition

Представим ситуацию:

![Диаграмма гонки](/03_cache/assets/18-race-condition.png)

У нас два запроса — A (UPDATE цены) и B (READ цены), плюс Redis и БД.

1. В момент `t=0` оба запроса стартуют.
   В БД цена товара `price = 900`.

2. Запрос B идёт в кэш:

   * `GET product:123` → MISS
   * он полез в БД, сделал `SELECT` и прочитал **старую цену 900**.

3. Пока B читает, запрос A успевает обновить данные:

   * `UPDATE price = 1000` в БД
   * потом по классике кэша делает `DELETE product:123` / инвалидацию.

4. После этого B, который всё ещё живёт в своём старом потоке событий,
   берёт только что прочитанное **старое значение 900** и делает:

   * `SET product:123 = { price: 900 }`.

Итог:
в БД уже **1000 ₽**, а в кэше мы **переписали назад 900 ₽** — чистая race condition между update и read+set.
...
Отдельный класс решений — внешние координаторы вроде ZooKeeper или etcd.
Они дают распределённые lock/lease и используются для более тяжёлых задач:
лидер-элекция, координация шардов, миграции, глобальные флаги.
Типичный паттерн: процесс создаёт “ephemeral”-запись (lock) в ZooKeeper/etcd, пока он жив — он считается единственным владельцем; как только процесс умирает или теряет сессию, lock освобождается автоматически.

В типичном веб-приложении для кэша этого оверкилл: вам почти всегда достаточно Redis-локов (Redlock/lease). ZK/etcd-локи чаще встречаются в больших Java/кластерах, базах, внутренних оркестраторах.
