...
На собесе достаточно понимать 3 подхода:
2pc, tcc и saga
...
TCC применяют там, где ошибка стоит дорого: биллинг, бронирования, управление квотами.
Минус — рост сложности: больше состояний, больше координации, нужны очистка протухших резервов 
...
- вызываем компенсацию в Inventory
- для биллинга в качестве компенсации
  - либо можно вернуть деньги если это возможно,
  - либо, если нам домен не позволяет автоматический возврат, сделать косвенную компенсацию: 
    отправить задачу менеджеру, сигнал в CRM - чтобы человек дальше разруливал операцию
...
- нет единой точки связности - сервиса который знает все обо всех
...
при хореографии часто возникает проблема dual write
...
1. Откатывать назад (rollback-first)
   ...
   Это похоже на привычную транзакцию, когда у нас все или ничего, но не всегда приемлемо для бизнеса

2. Дожимать вперёд (forward recovery)
   не откатываемся идеально, а доводим процесс до понятного исхода

   например, если деньги списались, а заказ не создался → ретраим создание заказа и если не вышло, то создаем тикет в CRM: менеджер далее изменит заказ или вернет деньги
...
зачем он нужен?
- хранит состояние процесса, то есть на каком шаге саги мы находимся
