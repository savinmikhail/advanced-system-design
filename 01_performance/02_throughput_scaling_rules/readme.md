...
Например
- У нас есть запрос на получение отзывов товара, работает за 200мс. Бизнесу не надо ускорять его до 100мс - пользователь не заметит.
...
- Примеры: ограниченный пул коннектов к Postgres; горячий ключ/строка с локом; ретраи из-за optimistic locking.
